...
def computeLists(circuit, index_best, number_of_populations, num_genes):
    ...
    for ind in list_pop:
        ...
        list_qubit_mutation.extend(
            [qr2[e] for e in range(0, num_genes) if e not in range(k, min(k + point, num_genes))])
        k = k + point
...
class Parameters:
    ...
    def __init__(self, pop_size, max_gen, epsilon_init, prob_mut, elitism, num_shots=1, progressBar=False, verbose=True,
                 draw_circuit=False, qobj_id=None):
...
class ReinforcementParameters(Parameters):
    ...
    def __init__(self, pop_size, max_gen, epsilon_init, epsilon, prob_mut, elitism=ELITISM_R, num_shots=1,
                 progressBar=False, verbose=True, draw_circuit=False, qobj_id=None):
        super().__init__(pop_size, max_gen, epsilon_init, prob_mut, elitism, num_shots, progressBar, verbose,
                         draw_circuit, qobj_id)
        self.epsilon = epsilon

    def __str__(self):
        return str(self.elitism) + "_eps_init_" + str(self.epsilon_init) + "_prob_mut_" + str(
            self.prob_mut) + "_eps_" + str(self.epsilon)
