...
class RealProblem(Problem):
    ...
    def getOptimum(self):
        ...
        for ind in list_index:
            ...
            for i in range(self.dim):
                l.append(
                    dis.convertFromFloatToBin(all_space[ind][i], self.lower_bounds[i], self.upper_bounds[i],
                                              self.num_bit_code))
    ...
    def getWholeSpace(self):
        ...
        for chr in sols:
            ...
            for i in range(self.dim):
                l.append(
                    dis.convertFromFloatToBin(chr[i], self.lower_bounds[i], self.upper_bounds[i], self.num_bit_code))
            gray_sols.append(l)
...
class VariationalProblem(RealProblem):
    ...
    def computeFitness(self, chr):
        ...
        self.iso_model.fit(data_train_iso)
        iso_predictions = self.iso_model.predict(data_val_iso)
        iso_predictions = [1 if pred == -1 else 0 for pred in iso_predictions]
        iso_predictions = np.array(iso_predictions, dtype=float)
        iso_true = self.y_train[mid_index:].values
...
class GriewangkProblem(RealProblem):
    ...
    def computeFitness(self, chr_real):
        res = 1 + sum([x ** 2 / 4000 for x in chr_real]) - math.prod(
            [math.cos(chr_real[i] / math.sqrt(i + 1)) for i in range(len(chr_real))])
        # print("fitness value ", res)
        return res


class AckleyProblem(RealProblem):
    def __init__(self, dim=2, num_bit_code=4, lower_bounds=[-32.768, -32.768], upper_bounds=[32.768, 32.768], a=20,
                 b=0.2, c=2 * math.pi):
    ...
    def computeFitness(self, chr_real):
        ...
        res = -self.a * math.exp(-self.b * math.sqrt(firstSum / self.dim)) - math.exp(
            secondSum / self.dim) + self.a + math.exp(1)
        # print("fitness value ", res)
        return res
...
class BirdProblem(RealProblem):
    def __init__(self, num_bit_code=4, lower_bounds=[-2 * math.pi, -2 * math.pi],
