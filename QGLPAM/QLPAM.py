...
if __name__ == "__main__":
    ...
    X_val_5perc = scaler.transform(X_val_5perc)


    def calculate_expectation_value(circuit, features, params):
        try:
            gpu_estimator = AerSimulator(method='statevector')
    ...
    reduced_data_val = pca.transform(X_val_5perc)
    ...
    ansatz = TwoLocal(dim, rotation_blocks=['rx', 'ry', 'rz'], entanglement_blocks=['cx', 'swap', 'h'],
                      entanglement='full', reps=1, insert_barriers=True)
    ...
    plt.savefig('graphics/{}/quantum_kernels.png'.format(ACTIVE_FOLDER))
    ...
    res_ae = optimizer.optimize(problem_ae, 3, max_gen=3, verbose=True, seed=42)
    res_oneclass = optimizer_1.optimize(problem_oneclass, 3, max_gen=3, verbose=True, seed=42)
    res_iso_forest = optimizer_2.optimize(problem_iso_forest, 3, max_gen=3, verbose=True, seed=42)
    print(res_oneclass)
    print(res_iso_forest)
    print(res_ae)

    pca_test_reduced = pca.transform(X_test_5perc)
    ...
    plt.grid()
    plt.savefig('graphics/{}/roc_curves_compared.png'.format(ACTIVE_FOLDER), dpi=300, bbox_inches='tight')
    plt.savefig('graphics/{}/roc_curves_comparison.png'.format(ACTIVE_FOLDER), dpi=300)
    plt.close()
